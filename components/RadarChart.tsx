...
const RadarChart: React.FC<RadarChartProps> = ({ data }) => {
  ...
  const polygonFillColor = "rgba(255, 255, 255, 0.2)";
  const polygonStrokeColor = "rgba(255, 255, 255, 0.8)";
  const pointFillColor = polygonStrokeColor;
  const pointStrokeColor = "rgba(255, 255, 255, 0.9)";
  ...
  return (
    <div className="w-full max-w-[200px] mx-auto">
        <svg viewBox={`0 0 ${size} ${size}`} className="w-full h-auto">
            ...
            {/* Labels */}
            <text x={center} y="12" textAnchor="middle" fill="white" fontSize="11" className="font-semibold">
                {data.codeReviews}%
                <tspan x={center} dy="1.2em" fontSize="10" className="font-normal opacity-80">Code review</tspan>
            </text>
            <text x={size - 8} y={center + 4} textAnchor="end" fill="white" fontSize="11" className="font-semibold">
                {data.issues}%
                <tspan x={size - 8} dy="1.2em" fontSize="10" className="font-normal opacity-80">Issues</tspan>
            </text>
            <text x={center} y={size - 8} textAnchor="middle" fill="white" fontSize="11" className="font-semibold">
                {data.pullRequests}%
                <tspan x={center} dy="1.2em" fontSize="10" className="font-normal opacity-80">Pull requests</tspan>
            </text>
            <text x="8" y={center + 4} textAnchor="start" fill="white" fontSize="11" className="font-semibold">
                {data.commits}%
                <tspan x="8" dy="1.2em" fontSize="10" className="font-normal opacity-80">Commits</tspan>
            </text>
        </svg>
    </div>
  );
